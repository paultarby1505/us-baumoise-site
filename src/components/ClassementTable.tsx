...
export default function ClassementTable({ lignes }: { lignes: ClassementLigne[] }) {
  const categoriesDisponibles = MATCH_CATEGORIES.filter((cat) =>
    lignes.some((l) => l.categorie === cat)
  );
  const [selected, setSelected] = useState<string>(categoriesDisponibles[0] ?? MATCH_CATEGORIES[0]);
  ...
  if (categoriesDisponibles.length === 0) {
    return <p className="text-foreground/60">Aucun classement publié pour le moment.</p>;
  }
