...
function SlotCard({ slot, joueur }: { slot: number; joueur: Joueur | undefined }) {
  return (
...
export default function CompositionPitch({ composition }: { composition: CompositionSlot[] }) {
  ...
  const hasRemplacants = REMPLACANTS_ROWS.flat().some((slot) => bySlot.has(slot));
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-4 rounded-lg bg-club-black p-4 sm:p-6">
        {COMPOSITION_ROWS.map((row, i) => (
          <div key={i} className="flex flex-wrap justify-center gap-3 sm:gap-4">
            {row.map((slot) => (
              <SlotCard key={slot} slot={slot} joueur={bySlot.get(slot)} />
            ))}
          </div>
        ))}
      </div>
      ...
      {hasRemplacants && (
        <div className="space-y-4 rounded-lg bg-club-black-soft p-4 sm:p-6">
